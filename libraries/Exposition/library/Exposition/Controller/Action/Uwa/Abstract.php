...
abstract class Exposition_Controller_Action_Uwa_Abstract extends Zend_Controller_Action
{
    ...
    protected function _setPreferenceValue($name, $value)
    {
        if (!isset($this->view->widget->preferences[$name])) {
            throw new Exposition_Controller_Action_Uwa_Exception('Unable to find preference "' . $name . '", please add preference before set a new value');
        }
    ...
    protected function _setWidgetMetaValue($meta, $value)
    {
        if (!isset($this->view->widget->metas[$meta])) {
            throw new Exposition_Controller_Action_Uwa_Exception('Illegal widget meta "' . $meta . '"');
        }
