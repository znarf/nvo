...
class Exposition_Archive_Zip extends Exposition_Archive_Abstract
{
    /**
     * Mime Type.
     *
    ...
    protected function _buildArchive()
    {
        ...
        if (!empty ($this->_options['sfx']))
            if ($fp = fopen($this->_options['sfx'], "rb")) {
                ...
                throw new Exposition_Archive_Exception(sprintf('Could not open sfx module from %s."', $this->_options['sfx']));
            }
        ...
        foreach ($this->_files as $current) {
            ...
            // files
                ...
                if (isset($current['data'])) {
                    ...
                    throw new Exposition_Archive_Exception(sprintf('Could not open file %s for reading. It was not added.', $this->_options['path']));
                }
    ...
    public function _extractArchive($outputDir)
    {
        if (!function_exists('zip_open')) {
            throw new Exposition_Archive_Exception('Unable to extract archive cause "zip_open" function is not available.');
        }
        ...
        while ($zipEntry = zip_read($zip)) {
            ...
            if (zip_entry_open($zip, $zipEntry, 'r')) {
                ...
                if ($this->_options['inmemory']) {
                    ...
                    throw new Exposition_Archive_Exception(sprintf('Unable to overwrite existing %s file cause overwrite options is disable', $completeName));
