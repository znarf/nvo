 ...
 */
class Exposition_Parser_Factory
{
    /**
    ...
    public static function getParser($type, $url, $cache = true)
    {
        switch ($type) {
            case 'uwa':
                // UWA XHTML Widget
                require_once 'Exposition/Parser/Uwa.php';
                return new Exposition_Parser_Uwa($url, $cache);
                break;
            default:
                // Exception
                throw new Exposition_Exception('Unrecognized type.');
                break;
        }
    }
}
