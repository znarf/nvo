...
abstract class Exposition_Archive_Abstract
{
    /**
     * Archive options
     *
    ...
    {
         throw new Exposition_Archive_Exception(sprintf('Function "%s" should overwrited by class %s', __FUNC__, __CLASS__));
    }
    ...
    public function createArchive()
    {
        ...
        if (!$this->_options['inmemory']) {
            ...
            if (!$this->_options['overwrite'] && file_exists($this->_options['path'])) {
                chdir($pwd);
                throw new Exposition_Archive_Exception(sprintf('File %s already exists.', $this->_options['path']));
            } else if ($this->_archive = fopen($this->_options['path'], 'wb+')) {
                chdir($pwd);
            } else {
                chdir($pwd);
                throw new Exposition_Archive_Exception(sprintf('Could not open %s for writing.', $this->_options['path']));
            }
    ...
    public function extractArchive($outputDir = null)
    {
        ...
        if (!is_readable($this->_options['path'])) {
            throw new Exposition_Archive_Exception(sprintf('Could not open file "%s"', $this->_options['path']));
        }
    ...
    protected function _extractArchiveToPath($outputDir)
    {
        ...
        if (!is_writable($outputDir)) {
            throw new Exposition_Archive_Exception(sprintf('Could not write on path "%s"', $outputDir));
        }
    ...
    public function addFile($filePath, $archiveFilePath = null)
    {
        if (file_exists($filePath)) {
            ...
            throw new Exposition_Archive_Exception(sprintf('Could not open file %s for reading.', $filePath));
        }
